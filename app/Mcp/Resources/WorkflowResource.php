...
#[Name('workflow')]
#[Title('Workflow')]
#[Description('Get an individual workflow by name for a workspace by kebab-slug for a project by UUID.')]
#[MimeType('application/json')]
class WorkflowResource extends Resource implements HasUriTemplate
{
    ...
    public function handle(Request $request): Response|ResponseFactory
    {
        ...
        /* @var \App\Data\WorkflowData|null $workflow */
        $workflow = rescue(
            fn () => app(WorkflowService::class)
                ->loadWorkflows($workspace->path)
                ->get($request->get('name'))
        );

        if (! $workflow) {
            return Response::error('Failed to load workflow.');
        }

        return $this->json($workflow->toMcpResource());
    }

    public function uriTemplate(): UriTemplate
    {
        return new UriTemplate('laborforest://projects/{uuid}/workspaces/{slugKebab}/workflows/{name}');
    }
}
