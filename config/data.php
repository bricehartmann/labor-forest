...
return [
    ...
    /*
     * Structure caching skips the PHP reflection analysis of data objects, but is
     * disabled on purpose here: `php artisan optimize` (native:build prebuild)
     * runs `data:cache-structures`, and the native runtime rebinds storage_path()
     * to "~/Library/Application Support/laborforest-dev/storage". A structure
     * cached there outlives the code it describes — `optimize:clear` from the
     * terminal writes to the project storage and never reaches it — so properties
     * added to app/Data classes silently vanish inside the app.
     */
    'structure_caching' => [
        'enabled' => false,
