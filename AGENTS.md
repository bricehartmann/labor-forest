...
- A settings file the test never wrote now reads as defaults instead of throwing, so `mcp_read_only` is `true` unless a test says otherwise — `LaborForestServerTest` writes `settingsYaml(['mcp_read_only' => false])` because `McpService::isReadOnly()` used to fail open on the missing disk and register every tool by accident.
